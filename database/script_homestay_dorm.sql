...
-- ============================================================
CREATE OR REPLACE FUNCTION fn_update_so_giuong_trong() RETURNS TRIGGER AS $$ BEGIN -- Cập nhật SoGiuongTrong: tổng số giường 'Chưa sử dụng' + 'Đang giữ chỗ'
UPDATE PHONG
SET SoGiuongTrong = (
        SELECT COUNT(*)
        FROM GIUONG
...
SET TrangThai = CASE
    ...
    END
WHERE MaPhong = COALESCE(NEW.MaPhong, OLD.MaPhong)
    AND TrangThai != 'Đang giữ chỗ';
-- Giữ nguyên nếu thủ công set là Đang giữ chỗ
RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
INSERT
    OR
UPDATE
    OR DELETE ON GIUONG FOR EACH ROW EXECUTE FUNCTION fn_update_so_giuong_trong();
-- ============================================================
-- 4. DỊCH VỤ
-- ============================================================
...
CREATE TABLE NHAN_VIEN (
    ...
    LoaiNV VARCHAR(30) NOT NULL CHECK (
        LoaiNV IN ('Quản lý', 'Sale', 'Kế toán', 'Kỹ thuật')
    )
);
-- ============================================================
-- 7. TÀI KHOẢN
...
CREATE TABLE KHACH_HANG (
    ...
    TrangThai VARCHAR(20) NOT NULL CHECK (
        TrangThai IN ('Mới', 'Chờ kiểm tra', 'Hợp lệ', 'Không hợp lệ')
    ),
    MaNhom VARCHAR(10) REFERENCES NHOM_KHACH_THUE(MaNhom),
    CONSTRAINT chk_ngay_cccd CHECK (
        NgayHetHanCCCD IS NULL
        OR NgayHetHanCCCD > NgayCapCCCD
    )
);
...
CREATE TABLE HOP_DONG (
    ...
    SoLuongGiuong INT NOT NULL CHECK (SoLuongGiuong > 0),
    KyThanhToan INT NOT NULL CHECK (KyThanhToan > 0),
    -- số tháng/kỳ
    NgayBatDau DATE NOT NULL,
    ThoiHanThue INT NOT NULL CHECK (ThoiHanThue > 0),
    -- số tháng
    GiaThue NUMERIC(12, 0) NOT NULL CHECK (GiaThue > 0),
    NgayKetThuc DATE,
    TrangThai VARCHAR(60) NOT NULL CHECK (
        TrangThai IN (
            'Mới',
            'Chưa xác nhận',
            'Đã ký xác nhận',
    ...
    MaKH VARCHAR(10) NOT NULL REFERENCES KHACH_HANG(MaKH),
    CONSTRAINT chk_ngay_hop_dong CHECK (
        NgayKetThuc IS NULL
...
CREATE TABLE THANH_TOAN (
    ...
    LoaiTT VARCHAR(30) NOT NULL CHECK (
        LoaiTT IN (
            'Tiền cọc',
            'Tiền thuê',
            'Phí dịch vụ',
    ...
    TrangThai VARCHAR(30) NOT NULL CHECK (
        TrangThai IN (
            'Mới',
            'Chờ thanh toán',
            'Đang thanh toán',
    ...
    PhuongThucThanhToan VARCHAR(50) CHECK (
        PhuongThucThanhToan IN ('Tiền mặt', 'Chuyển khoản', 'Thẻ')
    ),
    GhiChu TEXT,
...
CREATE TABLE BIEN_BAN_VI_PHAM (
    ...
    TrangThai VARCHAR(30) NOT NULL CHECK (
        TrangThai IN ('Mới', 'Đã xác nhận', 'Đã giải quyết')
    ),
...
CREATE TABLE TAI_SAN_TRANG_THIET_BI (
    ...
    MoTa TEXT,
    TrangThai VARCHAR(20) NOT NULL CHECK (
        TrangThai IN ('Đã bàn giao', 'Chưa bàn giao', 'Hỏng')
    ),
    MaPhong VARCHAR(10) NOT NULL REFERENCES PHONG(MaPhong)
);
-- ============================================================
...
CREATE TABLE BANG_DOI_SOAT (
    ...
    PhanTramHoanCoc NUMERIC(5, 2) CHECK (
        PhanTramHoanCoc BETWEEN 0 AND 100
    ),
...
VALUES (
        ...
        '123 Đinh Tiên Hoàng, P.1, Q.Bình Thạnh, TP.HCM',
        20,
        'Không nuôi thú cưng. Không hút thuốc trong phòng. Ra vào trước 23h.',
        'Đặt cọc 2 tháng tiền thuê.'
    ),
    (
        'CN02',
        'Chi nhánh Thủ Đức',
        '45 Võ Văn Ngân, P.Linh Chiểu, TP.Thủ Đức, TP.HCM',
        15,
        'Giữ yên lặng sau 22h. Không tụ tập đông người.',
        'Đặt cọc 1 tháng tiền thuê.'
    ),
    (
        'CN03',
        'Chi nhánh Gò Vấp',
        '78 Quang Trung, P.10, Q.Gò Vấp, TP.HCM',
        25,
        'Không để xe dưới lòng đường. Vệ sinh khu vực chung định kỳ.',
        'Đặt cọc 2 tháng tiền thuê.'
    ),
    (
        'CN04',
        'Chi nhánh Phú Nhuận',
        '12 Phan Xích Long, P.2, Q.Phú Nhuận, TP.HCM',
        10,
        'Không tiếp khách sau 22h. Không sử dụng bếp từ trong phòng.',
        'Đặt cọc 3 tháng tiền thuê.'
    );
-- ============================================================
-- PHÒNG
-- ============================================================
...
-- ---- PHÒNG TEST: tất cả giường đều "Chưa sử dụng" ----
('P012', 4, 'Nữ',  1300000, 'Còn giường trống', 4, 'CN01'),  -- Test Nguyên phòng (4 giường nữ, CN01)
('P013', 4, 'Nam', 1150000, 'Còn giường trống', 4, 'CN02'),  -- Test Ở ghép (4 giường nam, CN02)
('P014', 1, 'Nữ',  5500000, 'Còn giường trống', 1, 'CN03');  -- Test Nguyên phòng 1 giường (nữ, CN03)
...
-- ---- GIƯỜNG TEST: tất cả đều "Chưa sử dụng" ----
('G01', 'P012', 'Chưa sử dụng'),
('G02', 'P012', 'Chưa sử dụng'),
('G03', 'P012', 'Chưa sử dụng'),
('G04', 'P012', 'Chưa sử dụng'),
('G01', 'P013', 'Chưa sử dụng'),
('G02', 'P013', 'Chưa sử dụng'),
('G03', 'P013', 'Chưa sử dụng'),
('G04', 'P013', 'Chưa sử dụng'),
('G01', 'P014', 'Chưa sử dụng');

-- ============================================================
-- DỊCH VỤ
-- ============================================================
INSERT INTO DICH_VU (MaDV, TenDV, Gia)
VALUES ('DV01', 'Điện', 3500),
    ('DV02', 'Nước', 80000),
...
-- CHI NHÁNH - DỊCH VỤ
-- ============================================================
INSERT INTO CHI_NHANH_DICH_VU (MaCN, MaDV)
VALUES ('CN01', 'DV01'),
    ('CN01', 'DV02'),
    ('CN01', 'DV03'),
    ('CN01', 'DV04'),
...
-- NHÂN VIÊN
-- ============================================================
INSERT INTO NHAN_VIEN (MaNV, HoTen, SoDienThoai, Email, CCCD, LoaiNV)
VALUES (
        'NV01',
        'Nguyễn Thị Lan',
        '0901234567',
...
-- TÀI KHOẢN
-- ============================================================
INSERT INTO TAI_KHOAN (MaDangNhap, MatKhau, MaNV)
VALUES ('lan.nt', '$2b$10$abc...hash1', 'NV01'),
...
VALUES (
        ...
        '0901111111',
        '079200001111',
        'Cục CSQLHC về TTXH TP.HCM',
        '2020-01-01',
        '2030-01-01',
        'Việt Nam',
        '100 Lê Lợi, Q.1, TP.HCM',
        'Nhóm',
        'Hợp lệ',
        'NH01'
    ),
    ...
    (
        ...
        '2028-09-05',
        'Việt Nam',
        '66 An Dương Vương, Q.5',
        'Cá nhân',
        'Mới',
        NULL
    );
-- Cập nhật đại diện nhóm
UPDATE NHOM_KHACH_THUE
SET MaKHDaiDien = 'KH01'
WHERE MaNhom = 'NH01';
UPDATE NHOM_KHACH_THUE
SET MaKHDaiDien = 'KH02'
WHERE MaNhom = 'NH02';
UPDATE NHOM_KHACH_THUE
SET MaKHDaiDien = 'KH03'
WHERE MaNhom = 'NH03';
UPDATE NHOM_KHACH_THUE
SET MaKHDaiDien = 'KH04'
WHERE MaNhom = 'NH04';
UPDATE NHOM_KHACH_THUE
SET MaKHDaiDien = 'KH08'
WHERE MaNhom = 'NH05';
-- ============================================================
...
-- ---- PYC TEST cho màn hình Chi tiết ----
('YC09', 4, 1300000, '2026-06-01', 12, '2026-05-15 09:00', 'Nữ', '2026-05-10 08:00', 'Cần xác nhận', 'Nguyên phòng', 'NV02', 'KH02'), -- Test Nguyên phòng P012 (4 giường đều Chưa sử dụng)
('YC10', 2, 1150000, '2026-06-01',  6, '2026-05-15 10:00', 'Nam', '2026-05-10 08:30', 'Cần xác nhận', 'Ở ghép',       'NV04', 'KH05'), -- Test Ở ghép P013 (2/4 giường được chốt)
('YC16', 2, 1200000, '2026-07-01',  6, '2026-06-15 09:00', 'Nam', '2026-06-10 08:00', 'Cần xác nhận', 'Ở ghép',       'NV02', 'KH09'); -- Test Hủy thuê: P001 (giường Đang sử dụng)

-- ============================================================
-- CHI TIẾT PHIẾU YÊU CẦU
-- ============================================================
INSERT INTO CHI_TIET_PHIEU_YEU_CAU (MaYC, MaGiuong, MaPhong, TrangThaiChot)
VALUES ('YC01', 'G01', 'P001', 'Chốt'),
    ('YC01', 'G02', 'P001', 'Chốt'),
    ('YC01', 'G03', 'P001', 'Chốt'),
    ('YC01', 'G04', 'P001', 'Chốt'),
    ('YC02', 'G01', 'P002', 'Chốt'),
    ...
    ('YC05', 'G02', 'P007', 'Chốt'),
    ('YC06', 'G01', 'P010', 'Chốt'),
    -- YC07: Ở ghép, 1 giường G01 P006 đã chốt
    ('YC07', 'G01', 'P006', 'Chốt'),
    -- YC08: Nguyên phòng P014 (1 giường, Chưa sử dụng)
    ('YC08', 'G01', 'P014', 'Chốt'),
    -- YC09: Nguyên phòng P012 — tất cả 4 giường đã chốt
    ('YC09', 'G01', 'P012', 'Chốt'),
    ('YC09', 'G02', 'P012', 'Chốt'),
    ('YC09', 'G03', 'P012', 'Chốt'),
    ('YC09', 'G04', 'P012', 'Chốt'),
    -- YC10: Ở ghép P013 — 2 giường chốt, 2 không chốt
    ('YC10', 'G01', 'P013', 'Chốt'),
    ('YC10', 'G02', 'P013', 'Chốt'),
    ('YC10', 'G03', 'P013', 'Không chốt'),
    ('YC10', 'G04', 'P013', 'Không chốt'),
    -- YC16: Ở ghép P001 — 2 giường chốt (đều Đang sử dụng → test hủy thuê)
    ('YC16', 'G01', 'P001', 'Chốt'),
    ('YC16', 'G02', 'P001', 'Chốt');
-- ============================================================
-- HỢP ĐỒNG
...
-- HỢP ĐỒNG - GIƯỜNG
-- ============================================================
INSERT INTO HOP_DONG_GIUONG (MaHD, MaGiuong, MaPhong)
VALUES ('HD01', 'G01', 'P001'),
    ('HD01', 'G02', 'P001'),
    ('HD01', 'G03', 'P001'),
    ('HD01', 'G04', 'P001'),
    ('HD02', 'G01', 'P002'),
...
VALUES (
        'VP01',
        'Nuôi chó trong phòng trọ, vi phạm nội quy',
        '2025-02-15',
        'Đã giải quyết',
        500000,
        'HD01'
    ...
    (
        'VP02',
        'Gây tiếng ồn sau 23h nhiều lần, hàng xóm khiếu nại',
        '2025-03-10',
        'Đã xác nhận',
        300000,
        'HD02'
    ...
    (
        'VP03',
        'Tự ý khoan tường lắp điều hòa khi chưa có phép',
        '2025-01-20',
        'Đã giải quyết',
        1000000,
        'HD03'
...
-- LỊCH TRẢ PHÒNG
-- ============================================================
INSERT INTO LICH_TRA_PHONG (MaLichTraPhong, Ngay, Gio, TrangThai, MaHD, MaNV)
VALUES (
        'LTP01',
        '2025-06-01',
        '09:00',
...
-- ============================================================
INSERT INTO TAI_SAN_TRANG_THIET_BI (MaVatDung, TenVatDung, MoTa, TrangThai, MaPhong)
VALUES (
        'VD01',
        'Giường tầng đôi',
        'Giường sắt 2 tầng, có nệm',
        'Đã bàn giao',
        'P001'
    ),
    (
...
VALUES (
    ...
    (
        'BB03',
        '2025-01-10 09:30',
        'Bàn giao nguyên phòng, đầy đủ thiết bị',
        'Đã ký xác nhận',
        NULL,
        'HD03',
        'NV05'
    ...
    (
        'BB04',
        '2025-02-01 08:30',
        'Bàn giao đầy đủ, bình nóng lạnh hỏng',
        'Đã ký xác nhận',
        NULL,
        'HD04',
        'NV05'
...
-- ============================================================
INSERT INTO BAO_CAO_TINH_TRANG_PHONG (
        MaBaoCao,
        NgayLap,
        KetQuaVeSinh,
        DaKiemTraHD,
...
-- KHOẢN KHẤU TRỪ
-- ============================================================
INSERT INTO KHOAN_KHAU_TRU (MaDoiSoat, TenKhoan, SoTien)
VALUES (
        'DS01',
        'Phí vệ sinh phòng (tình trạng kém)',
        300000
    ),
    (
        'DS01',
        'Tiền điện tháng cuối chưa thanh toán',
        220000
    ),
    (
        'DS02',
...
FROM TAI_KHOAN
UNION ALL
SELECT 'NHOM_KHACH_THUE',
    COUNT(*)
FROM NHOM_KHACH_THUE
UNION ALL
SELECT 'KHACH_HANG',
    COUNT(*)
FROM KHACH_HANG
UNION ALL
SELECT 'PHIEU_YEU_CAU_XEM_PHONG',
    COUNT(*)
FROM PHIEU_YEU_CAU_XEM_PHONG
UNION ALL
SELECT 'CHI_TIET_PHIEU_YEU_CAU',
    COUNT(*)
...
FROM THANH_TOAN
UNION ALL
SELECT 'BIEN_BAN_VI_PHAM',
    COUNT(*)
FROM BIEN_BAN_VI_PHAM
UNION ALL
SELECT 'LICH_TRA_PHONG',
    COUNT(*)
FROM LICH_TRA_PHONG
UNION ALL
SELECT 'TAI_SAN_TRANG_THIET_BI',
    COUNT(*)
FROM TAI_SAN_TRANG_THIET_BI
UNION ALL
SELECT 'BIEN_BAN_BAN_GIAO',
    COUNT(*)
FROM BIEN_BAN_BAN_GIAO
UNION ALL
SELECT 'BAO_CAO_TINH_TRANG_PHONG',
    COUNT(*)
FROM BAO_CAO_TINH_TRANG_PHONG
UNION ALL
SELECT 'BIEN_BAN_TRA_PHONG',
    COUNT(*)
FROM BIEN_BAN_TRA_PHONG
UNION ALL
SELECT 'BANG_DOI_SOAT',
    COUNT(*)
FROM BANG_DOI_SOAT
UNION ALL
SELECT 'KHOAN_KHAU_TRU',
    COUNT(*)
FROM KHOAN_KHAU_TRU
UNION ALL
SELECT 'CHI_TIET_BAN_GIAO',
    COUNT(*)
